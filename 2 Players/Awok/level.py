...
class Level:
    def __init__(self):
        ...
        self.coin_group2 = pygame.sprite.Group()
        self.coin_collision_sprites2 = pygame.sprite.Group()
    ...
    def setup(self):
        tmx_data = load_pygame('../Awok/data/Tes2 anyar.tmx')
        
        # Collion tiles
        for obj in tmx_data.get_layer_by_name('Tembok2'):
            Collosion((obj.x, obj.y), obj.image,  self.collision_sprites)
    
        #pos
        pos1_frames = import_folder('../Awok/graphics/PosP1')
        ...
        pos2_frames = import_folder('../Awok/graphics/PosP1')
        ...
        pos3_frames = import_folder('../Awok/graphics/PosP2')
        ...
        pos4_frames = import_folder('../Awok/graphics/PosP2')
        ...
        pos5_frames = import_folder('../Awok/graphics/PosP2')
        ...
        coin_frames2 = import_folder('../Awok/graphics/Coin2')
        for obj in tmx_data.get_layer_by_name('coin2'):
            self.coin_sprites = Coin((obj.x,obj.y), coin_frames2, [self.all_sprites, self.coin_collision_sprites2])
        ...
        for obj in tmx_data.get_layer_by_name('Player'):
            if obj.name == 'Start':
                self.player = Player(
                    ...
                    coin_collision_sprites2 = self.coin_collision_sprites2,
        ...
        Generic(
            pos = (0,0), 
            surf = pygame.image.load('../Awok/World/MapSMAMissionZoom4xBaru.png').convert_alpha(),
            groups = self.all_sprites,
            z = LAYERS['ground'])
        # print(self.player.update.coin)

    def finish(self, kebenaran):
        self.player2.kebenaran = kebenaran
        # self.kebenaran = kebenaran
        # print(self.player.kebenaran)
        if self.player2.kebenaran == True:
            
            # print('tes')
            self.screen.blit(BG, (0, 0))
            self.ui.show_coins(self.player2.coin)
            self.ui.show_coins2(self.player.coin2)
    ...
    def get_username(self, text):
        self.ikan = text
        self.ayam = 'Nama Peserta MISSION: ' + text
    ...
    def run(self,dt):
        ...
        self.finish(self.player2.kebenaran)
